...
title: data-model
sources:
  - llika-core/src/model.rs
  - llika-core/src/io.rs
covers: >
...
`llika-core/src/io.rs:InputSchema` is the whole input file: `stations` and `lines`,
both `Vec`, both deserialized by serde. `llika-core/src/model.rs:Station` carries
`id`, `name`, `lat`, `lon`; `llika-core/src/model.rs:Line` carries `id`, `name`,
`color` and an **ordered** station id list.

`llika-core/src/model.rs:Network` wraps a `petgraph` `UnGraph<usize, LineSet>`.

**The edge is the corridor.** `llika-core/src/io.rs:from_input` walks each line's
...
`llika-core/src/model.rs:degree` counts corridors: a station where two lines run
side by side is degree 2, not degree 4. `llika-core/src/model.rs:lines_at_station`
unions the line sets of the incident edges;
`llika-core/src/model.rs:lines_between` reads one corridor's set.
...
`llika-core/src/model.rs:LineSet` is a `BTreeSet<usize>` of line indices. The
...
`llika-core/src/io.rs:InputError` has exactly five variants, all hard errors, none
...
`llika-core/src/io.rs:parse_input`, deliberately a different type from the five
semantic conditions.
